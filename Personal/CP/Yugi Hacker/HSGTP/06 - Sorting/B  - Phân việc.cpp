...
#elif defined(LOCAL) || defined(RELEASE)
...
void Solve()
{
    ...
    sort(allr(a, 1, 2 * n),
         [](pii &i, pii &j)
         {
             int x = i.fi - i.se, y = j.fi - j.se;
             return (x != y ? x < y : i < j);
         });
    for (int i = 1; i <= n; i++) { ans += a[i].fi; }
    for (int i = n + 1; i <= 2 * n; i++) { ans += a[i].se; }
    return;
}
