...
- 💾 Repository Locations:
    🌐 Remote Repository   : https://github.com/Macphuckhang11112007/Code/tree/main/CP\TopAlgo\Month_01\Lesson_04\03_AC3_HR.cpp
    🖥️ Local Repository    : C:/Users/macph/Downloads/MPK/Code/CP/TopAlgo/Month_01/Lesson_04/03_AC3_HR.cpp
- 📄 File Name             : 03_AC3_HR.cpp
- 🗓️ Creation Date         : 16:08 08-07-2025
- 💻 Coding Time           : HH:MM
...
signed main ()
{
    ...
    while (tc--)
    {
        ...
        sort (a + 1, a + n + 1);
        for (int i = 1; i <= k; i++) pq.emplace (a [i].se);
        for (int i = k; i <= n; i++)
        {
            if (i > k && pq.top () < a [i].se)
            {
                pq.pop ();
                pq.emplace (a [i].se);
            }
            int r = min (pq.top (), a [i].se);
            ans = max (ans, r - a [i].fi + 1);
        }
        cout << ans << endl;
    }
    return 0;
