...
- 💾 Repository Locations:
    🌐 Remote Repository   : https://github.com/Macphuckhang11112007/Code/tree/main/CP\TopAlgo\Month_01\Lesson_03\3_Max-Value-of-Equation_LC.cpp
    🖥️ Local Repository    : C:/Users/macph/Downloads/MPK/Code/CP/TopAlgo/Month_01/Lesson_03/3_Max-Value-of-Equation_LC.cpp
- 📄 File Name             : 3_Max-Value-of-Equation_LC.cpp
- 🗓️ Creation Date         : 21:54 07-07-2025
- 💻 Coding Time           : HH:MM
...
2. 🎯 PROBLEM STATEMENT
─────────────────────────────────────────────────────────────────
- 🌟 Problem Title         : 3_Max-Value-of-Equation_LC
...
signed main ()
{
    ...
    while (tc--)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            while (!dq.emp () && a [i].x - a [dq.fr ()].x > k) dq.ppf ();
            if (!dq.emp ()) ans = max (ans, p [i] + q [dq.fr ()]);
            while (!dq.emp () && q [dq.bk ()] < q [i]) dq.ppb ();
            dq.eb (i);
        }
        cout << ans;
    }
