...
- 💾 Repository Locations:
    🌐 Remote Repository   : https://github.com/Macphuckhang11112007/Code/tree/main/CP\TopAlgo\Month_01\Lesson_05\02_Movie-Festival-II_CSES.cpp
    🖥️ Local Repository    : C:/Users/macph/Downloads/MPK/Code/CP/TopAlgo/Month_01/Lesson_05/02_Movie-Festival-II_CSES.cpp
- 📄 File Name             : 02_Movie-Festival-II_CSES.cpp
- 🗓️ Creation Date         : 15:33 11-07-2025
- 💻 Coding Time           : HH:MM
...
2. 🎯 PROBLEM STATEMENT
─────────────────────────────────────────────────────────────────
- 🌟 Problem Title         : 02_Movie-Festival-II_CSES
...
signed main ()
{
    ...
    while (tc--)
    {
        ...
        sort (a + 1, a + n + 1, Comp);
        for (int i = 1; i <= k; i++) ms.emplace (0);
        for (int i = 1; i <= n; i++)
        {
            auto p = ms.upper_bound (a [i].fi);
            if (p != ms.begin ())
            {
                p--;
                ms.erase (p);
