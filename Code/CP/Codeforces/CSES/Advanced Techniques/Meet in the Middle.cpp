...
void BT(int id, int &idx, int l, int r, ll sum) {
    if (sum > s) {
        return;
    } else if (sum == s) {
        cnt[id][++idx] = sum;
...
void Solve() {
    ...
    int i = 1, j = 1;
    sort(allr(cnt[1], 1, p));
    sort(allr(cnt[2], 1, q), greater<ll>());
    while (i <= p && j <= q) {
        ll x = cnt[1][i], y = cnt[2][j];
        if (x + y < s) {
            i++;
        } else if (x + y > s) {
            j++;
            ...
            ll c1 = 1, c2 = 1;
            while (i < p && cnt[1][i] == cnt[1][i + 1]) {
                i++;
                c1++;
            }
            while (j < q && cnt[2][j] == cnt[2][j + 1]) {
                j++;
                c2++;
            }
            i++;
