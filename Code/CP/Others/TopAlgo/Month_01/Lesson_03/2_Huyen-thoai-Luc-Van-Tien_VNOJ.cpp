...
- 💾 Repository Locations:
    🌐 Remote Repository   : https://github.com/Macphuckhang11112007/Code/tree/main/CP\TopAlgo\Month_01\Lesson_03\2_Huyen-thoai-Luc-Van-Tien_VNOJ.cpp
    🖥️ Local Repository    : C:/Users/macph/Downloads/MPK/Code/CP/TopAlgo/Month_01/Lesson_03/2_Huyen-thoai-Luc-Van-Tien_VNOJ.cpp
- 📄 File Name             : 2_Huyen-thoai-Luc-Van-Tien_VNOJ.cpp
- 🗓️ Creation Date         : 18:59 07-07-2025
- 💻 Coding Time           : HH:MM
...
2. 🎯 PROBLEM STATEMENT
─────────────────────────────────────────────────────────────────
- 🌟 Problem Title         : 2_Huyen-thoai-Luc-Van-Tien_VNOJ
...
signed main ()
{
    ...
    while (tc--)
    {
        ...
        for (int i = 1; i <= k; i++)
        {
            while (!dq.emp () && a [dq.bk ()] > a [i]) dq.ppb ();
            dq.eb (i);
        }
        cout << a [dq.fr ()] << " ";
        for (int i = k + 1; i <= n; i++)
        {
            while (!dq.emp () && dq.fr () < i - k + 1) dq.ppf ();
            while (!dq.emp () && a [dq.bk ()] > a [i]) dq.ppb ();
            dq.eb (i);
            cout << a [dq.fr ()] << " ";
        }
