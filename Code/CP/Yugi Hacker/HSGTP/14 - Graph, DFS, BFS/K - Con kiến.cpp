...
// -------------------- [Entry Point] --------------------
const int N = 1e3, MOD = 1e8 + 7;

int n, m;
int a[N + 1][N + 1];
int vst[N + 1][N + 1], f[N + 1][N + 1];
pii dir[] = {{0, 0}, {0, -1}, {1, 0}, {0, 1}, {-1, 0}};
        ...
        while (sz--) {
            ...
            for (int i = 1; i <= 4; i++) {
                ...
                if (1 <= v.fi && v.fi <= n && 1 <= v.se && v.se <= m && a[v.fi][v.se]) {
                    if (!vst[v.fi][v.se]) {
                        vst[v.fi][v.se] = vst[u.fi][u.se] + 1;
                        (f[v.fi][v.se] = f[u.fi][u.se]) %= MOD;
                        q.ep(v);
                    } else if (vst[u.fi][u.se] + 1 == vst[v.fi][v.se]) {
                        (f[v.fi][v.se] += f[u.fi][u.se]) %= MOD;
                    }
                }
            }
