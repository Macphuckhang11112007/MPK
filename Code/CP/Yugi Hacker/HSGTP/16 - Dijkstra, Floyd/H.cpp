...
// ==================== [Entry Point] ====================
const ll N = 100, ST = 1 << 4;

ll                 n, s1, s2, s3, s4, ans = LLMAX;
vector<ll>         src;
vector<tiii>       a[N + 1];
vector<vector<ll>> dist(N + 1, vector<ll>(ST + 1, LLMAX));
bitset<ST + 1>     vis[N + 1];
pqueue_min<tlll>   pq;
...
void Dijkstra() {
    ...
    while (sz(pq)) {
        ...
        for (auto [v, w, st] : a[u]) {
            ll x = du + w;
            if (dist[v][i] > x) {
                pq.ep(dist[v][i] = x, v, i);
            }
            for (int id = 0; id < (1 << 4); id++) {
                x = du + dist[u][id];
                if (dist[u][i | id] > x) {
                    pq.ep(dist[u][i | id] = x, u, i | id);
                }
            }
        }
    }
