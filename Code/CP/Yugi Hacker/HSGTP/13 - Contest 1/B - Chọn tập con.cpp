...
int n, s;
int a[N + 1];
ll  s_a[(1 << (N / 2)) + 1], s_b[(1 << (N / 2)) + 1];
int id_a = 0, id_b = 0;
ll  ans;

void BT_a(int i = 1, ll sum = 0) {
    if (i > n / 2) {
        s_a[++id_a] = sum;
        return;
    }
...
void BT_b(int i = n / 2 + 1, ll sum = 0) {
    if (i > n) {
        s_b[++id_b] = sum;
        return;
    }
...
void Solve() {
    ...
    BT_a();
    BT_b();
    sort(s_a + 1, s_a + id_a + 1);
    sort(s_b + 1, s_b + id_b + 1, greater<ll>());
    int p = 1, q = 1;
    while (p <= id_a && q <= id_b) {
        ll sum = s_a[p] + s_b[q];
        if (sum == s) {
            ...
            do {
                c_a++;
                p++;
            } while (p <= id_a && s_a[p] == s_a[p - 1]);
            do {
                c_b++;
                q++;
            } while (q <= id_b && s_b[q] == s_b[q - 1]);
            ans += 1LL * c_a * c_b;
        }
        elif (sum < s) {
            p++;
